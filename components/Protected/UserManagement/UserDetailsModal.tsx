...
import { X, Calendar, Mail, ArrowUp, ArrowDown, PlayCircle, PauseCircle } from "lucide-react";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-3xl shadow-xl w-full max-w-6xl h-[90vh] overflow-y-auto relative flex flex-col md:flex-row overflow-hidden">
        ...
        <button
          onClick={onClose}
          className="absolute right-1 top-1 z-10 md:hidden p-2 bg-white/80 rounded-full hover:bg-gray-100 transition-colors"
        >
        ...
        <div className="hidden md:flex flex-col w-full md:w-[65%] lg:w-[70%] bg-white p-6 md:p-8 overflow-hidden relative">
          <button
            onClick={onClose}
            className="absolute right-2 top-2 p-2 hover:bg-red-50 rounded-full transition-colors cursor-pointer"
          >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-8">
            {isCreator && "total_earning" in data ? (
              <>
                <StatBox
                  label="Total earning"
                  value={`$${data.total_earning?.toLocaleString() || 0}`}
                  isUp={true}
                  trend="8%"
                />
                <StatBox
                  label="Total videos"
                  value={data.total_videos?.toString() || "0"}
                />
                <StatBox
                  label="Total views"
                  value={data.total_views?.toLocaleString() || "0"}
                  isUp={true}
                  trend="12%"
                />
              </>
...
function VideoCard({ video }: { video: import("@/types/userManagement.types").CreatorVideo }) {
    ...
    return (
        <div className="flex flex-col p-3 border border-[#EAECF0] rounded-2xl hover:border-primary/30 transition-all cursor-pointer group" onClick={() => !isPlaying && setIsPlaying(true)}>
                ...
                <div className="flex items-start justify-between">
                    <div>
                        ...
                        <div className="flex items-center gap-2 mt-1 flex-wrap">
                            <span className="text-xs text-secondary font-medium">{video.product_shade || "Default"}</span>
                            {video.product_colour_hex && (
                                <div className="w-2 h-2 rounded-full border border-gray-200" style={{ backgroundColor: video.product_colour_hex }} />
                            )}
                            <span className="text-xs text-[#475467]">{video.product_category || "Category"}</span>
                        </div>
                    </div>
                    <Button 
                        variant="ghost" 
                        size="sm" 
                        className="h-8 w-8 p-0 shrink-0 rounded-full hover:bg-gray-100"
                        onClick={togglePlay}
                    >
                        {isPlaying ? (
